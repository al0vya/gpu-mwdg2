...
)
{
	...
	if ( read_keyword_str(input_filename, "hwfv1", 5) )
	{
		solver_params.solver_type = HWFV1;
		solver_params.CFL         = C(0.5);
	}
	else if ( read_keyword_str(input_filename, "mwdg2", 5) )
	{
	...
	{
		fprintf(stderr, "Error: invalid solver type specified, please specify either \"hwfv1\" or \"mwdg2\", file: %s, line: %d.\n", __FILE__, __LINE__);
		exit(-1);
	}

	solver_params.grading = read_keyword_str(input_filename, "grading", 7);
    
	solver_params.limitslopes = read_keyword_str(input_filename, "limitslopes", 11);
	...
	solver_params.refine_wall = read_keyword_str(input_filename, "refine_wall", 11);
